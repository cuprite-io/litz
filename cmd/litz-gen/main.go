...
func generateCode(pkgName string, structs []StructInfo) ([]byte, error) {
	...
	fmt.Fprintf(&buf, "// Code generated by litz-gen. DO NOT EDIT.\npackage %s\n\n", pkgName)
	...
	for _, s := range structs {
		...
		buf.WriteString("\ttotalSize := fixedSize\n\n")
		...
		fmt.Fprintf(&buf, "\t*(*%s)(unsafe.Pointer(&buf[0])) = %s{}\n", mirrorName, mirrorName)
		fmt.Fprintf(&buf, "\tmirror := (*%s)(unsafe.Pointer(&buf[0]))\n\n", mirrorName)
		...
		if hasVariableFields {
			buf.WriteString("\tcurrentOffset := uintptr(fixedSize)\n\n")
		}
		...
		fmt.Fprintf(&buf, "\tfixedSize := int(unsafe.Sizeof(%s{}))\n", mirrorName)
		buf.WriteString("\tif len(buf) < fixedSize {\n")
		fmt.Fprintf(&buf, "\t\treturn %sErrBufferTooShort\n", prefix)
		buf.WriteString("\t}\n\n")

		fmt.Fprintf(&buf, "\tmirror := (*%s)(unsafe.Pointer(&buf[0]))\n\n", mirrorName)

		// Bounds checking utilizing sentinel errors
		for _, f := range s.Fields {
			switch f.Kind {
			case TypeString:
		...
		// Assign Primitives individually
		for _, f := range s.Fields {
			if f.Kind == TypePrimitive {
